...
namespace {
  ...
  cl::opt<std::string>
  LoadLocations("load-locations", cl::desc("..."));
}
...
bool parseLocation(std::string option,
                   std::string &fname,
...
void parseLoadLocations(Module *module,
                        std::string parameter,
                        std::vector<Interpreter::LineLocation> &result) {
  ...
  while (std::getline(stream, token, ',')) {
    std::vector<unsigned int> lines;
    if (!parseLocation(token, fname, lines)) {
    ...
    result.push_back(Interpreter::LineLocation(fname, lines));
  }
}
...
int main(int argc, char **argv, char **envp) {
  ...
  std::vector<Interpreter::LineLocation> loadLocations;
  parseLoadLocations(mainModule, LoadLocations, loadLocations);

  Interpreter::InterpreterOptions IOpts;
  IOpts.MakeConcreteSymbolic = MakeConcreteSymbolic;
  IOpts.loadLocations = loadLocations;
