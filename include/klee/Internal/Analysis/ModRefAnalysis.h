...
public:
  ...
  ModRefAnalysis(llvm::Module *module,
  ...
  void getApproximateModInfos(llvm::Instruction *inst,
                              AllocSite hint,
...
private:
  ...
  void addStore(llvm::Function *f, llvm::Instruction *inst);

  bool canIgnoreStackObject(llvm::Function *f, const llvm::Value *value);

  void collectRefInfo(llvm::Function *entry);

  void addLoad(llvm::Function *f, llvm::Instruction *inst);

  void addOverridingStore(llvm::Instruction *inst);
